...
function Turdify({ nfts, closeModal, collection, deturdify, refresh }) {
  ...
  async function overlay() {
    const baseImage = await Jimp.read(nfts[0].metadata.image);
      baseImage
        .resize(500, 500)
        .quality(60)
        .greyscale();

    const overlayImagePath = file
    ...
    const newFile = new File([blob], 'output.png', { type: 'image/png' });
    setFile(newFile)
                ...
                <div className={styles.turdifyLeft}>
                  {
                    type === 'mask' && (
                      <>
                        ...
                        <FormGroup>
                          <FormControlLabel control={<Switch checked={isOverlay} onChange={(e, val) => setIsOverlay(val)} />} label="Transparent overlay" />
                        </FormGroup>
